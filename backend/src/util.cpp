...
map<string, string> carregarDicionario(bool inverso) {
    map<string, string> dicionario;
    ifstream arquivo(DICIONARIO_ARQUIVO);
    string linha;
    while (getline(arquivo, linha)) {
        size_t pos = linha.find(';');
        if (pos != string::npos) {
            std::string chave = toLowerCase(linha.substr(0, pos));
            string valor = toLowerCase(linha.substr(pos + 1));
            if (inverso)
...
void salvarHistorico(const string& entrada, const string& traducao) {
    ...
    if (arquivo.is_open()) {
        ...
        cerr << aplicarCor("Erro ao salvar histórico.\n", "vermelho");
    }
}

vector<string> carregarHistorico() {
    return carregarLista(HISTORICO_ARQUIVO);
}
...
void mostrarHistorico() {
    ...
    if (historico.empty()) {
        cout << "Nenhuma tradução no histórico.\n";
        return;
    }
    int i = 1;
